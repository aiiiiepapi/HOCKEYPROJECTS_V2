...
| Raw data lake — Magnus | SCAFFOLDED 2026-08-02: v1 April-2026 scrape (264 PDFs 25-26, FFHG HockeyNet feuilles de match) carries over verbatim ONCE LOCATED (not in OneDrive — Seb to point at server/C:\dev copy). Fetcher shipped: tools/fetch_magnus.py (stdlib, PC/server-side; hockeynet.fr robots-blocks the cloud) — range/ids/probe-new modes, sha256 manifest; .bat in _manager\v2_bootstrap. v1 quirk docs read + adopted as reference: CV flag 67% unreliable (Joueurs list is EN authority), Type field unreliable (strength from penalty Debut/Fin windows, misconducts excluded), 25% goalie jersey collisions (positional A/B fix), multi-page PDFs, NO explicit pull events -> TOI back-calc (4 cases) + two-factor confirmation. v2 build order: lake -> GT hand-trace ~10 games -> magnus adapter (PDF->interval dict, shared engine rule 15) -> gates + random audit -> coach map (PDFs likely lack coaches; hand-curate 12 teams) -> ledger/profiles/morning sheet. Open: prior seasons availability (probe), 26-27 nightly fetch joins Sept automation | 2026-08-02 |
