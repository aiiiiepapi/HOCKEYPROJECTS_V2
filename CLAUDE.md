...
| AHL fix status | Round 2 (2026-08-01): oracle test proves levels are the failure AND fixable in principle (calibration recovers with true levels; marg4 stays ~0-skill -> AHL -3.5 permanently no-bet). But the moving level is 6v5-specific — team-environment anchor tested leak-free and REFUTED (scales ~1.0, no change); market-total anchoring refuted by implication. Surviving path: 26-27 re-litigation gate + paper-trade harness with real lines + coach-delta product design. rate_scale param added to pricer (gated, off by default) | 2026-08-01 |
