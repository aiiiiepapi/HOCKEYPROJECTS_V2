"""
backtest_survival.py — coach-conditional hazard law, walk-forward validation.
...
Everything else identical to backtest.py. Writes backtest_rows_survival.json.
"""
import json
import math
import sys
...
def k_coach(name):
    p = min(max(p_coach(name), 0.02), 0.97)
    return min(max(math.log(1.0 - p) / LOG_S_LEAGUE, 0.15), 5.0)
...
def cached_price(R, pulled, strength, m):
    key = (R, pulled, strength, round(m, 1))
    if key not in _cache:
        _cache[key] = MP.price(R, pulled0=pulled, strength0=strength,
                               m_coach=round(m, 1), n=20000, seed=13, coach_shift=0)
    return _cache[key]
...
def main():
    ...
    for inst in insts:
        ...
        km = k_coach(inst["trailing_coach"])
    ...
    json.dump(rows, open(DER / "backtest_rows_survival.json", "w"))
