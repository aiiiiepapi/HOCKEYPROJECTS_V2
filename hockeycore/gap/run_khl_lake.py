...
def _canon(name):
    """Coach identity key = 'Surname Firstname' (first two tokens).
    MANAGER ADJUDICATION 2026-08-08 (handoff open item 2): 2025+ pages add
    patronymics — 24 coaches split across the 2025 boundary otherwise
    (Никитин Игорь == Никитин Игорь Валерьевич). Census shows ZERO
    two-token collisions between distinct people across all 4 seasons, so
    stripping is loss-free. Adapter stays verbatim (rule: identity fixes
    live in the runner — SHL Lindholm/Lindbom precedent)."""
    if name is None:
        return None
    ...
    for season in SEASONS:
        ...
        for f in sorted((LAKE / season).glob("*_text.html"),
            ...
            for i in extract_instances(g, 3):
                coach = _canon(g["coaches"][i["trailing"]])
                if coach is None:
                ...
                i["coach"] = coach
                i["leader_coach"] = _canon(
                    g["coaches"]["home" if i["trailing"] == "away" else "away"])
                rows.append(i)
                n_season += 1
